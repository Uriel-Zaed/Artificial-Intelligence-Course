...
class A_Star_Mis():
    def __init__(self, initial_state, goal_state, N, weight):
        ...
        self.root = Node(initial_state, None, 0, self.huer_Mis(initial_state))
        self.solution = None
    ...
    def get_childs(self, cur_node):
        ...
        if em_sq_i-1>=0: # up to down
            ...
            child_list.append(Node(child_state, cur_node, cur_node.g_n+1,self.huer_Mis(child_state)))
    ...
    # out of row and out of col
    def huer_Mis(self, state: State):
        sum_h = 0
        for i in range(len(state.board)):
            for j in range(len(state.board[i])):
                if state.board[i][j] != 0:
                    g_i, g_j = self.find_goal_square(state.board[i][j])
                    sum_h += self.miss_placed(i, j, g_i, g_j)
        return sum_h
    
    def miss_placed(self, i, j, g_i, g_j):
        sum = 0 
        if i != g_i:
            sum += 1
        if j != g_j:
